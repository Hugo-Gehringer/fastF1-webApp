<template>
  <div class="f1-component-container">
    ...
    <div v-if="sessionData" class="f1-results-container">
        ...
        <div class="f1-header-info">
          <div>
            ...
            <p class="f1-header-description">
              {{ sessionData.circuit_name || searchParams.circuit?.toUpperCase() }} - {{ sessionData.country }} {{ sessionData.year }}
            </p>
          </div>
          <div class="telemetry-display">
        ...
        <div class="f1-card-content">
          ...
          <div class="f1-data-grid-4">
            <div class="f1-metric-card" style="--accent-color: #00D2BE;">
              ...
              <div class="f1-metric-subtitle">Current Session</div>
            </div>
            ...
            <div class="f1-metric-card" style="--accent-color: #C0C0C0;">
              ...
              <div class="f1-metric-value value-silver">
                {{ sessionData.circuit_name || sessionData.circuit || 'N/A' }}
              </div>
              <div class="f1-metric-subtitle">{{ sessionData.location || 'Circuit' }}</div>
            </div>
            ...
            <div class="f1-metric-card" style="--accent-color: #00FF7F;">
              ...
              <div class="f1-metric-subtitle">Session Start</div>
            </div>
            ...
            <div class="f1-metric-card" style="--accent-color: #00F5FF;">
              ...
              <div class="f1-metric-subtitle">Session Laps</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- Podium Summary -->
      <div v-if="sessionData.drivers.length >= 3" class="mt-8 p-6 bg-gradient-to-r from-mercedes-black/50 to-mercedes-charcoal/50 rounded-xl border border-mercedes-dark-silver/30">
      ...
      <div v-if="sessionData.drivers && sessionData.drivers.length > 0" class="f1-card">
        <div class="f1-card-content">
          <h4 class="f1-section-title">
            ...
            RACE CLASSIFICATION ({{ sessionData.drivers.length }})
          </h4>

          <div class="space-y-3">
            <div v-for="(driver, index) in sessionData.drivers" :key="driver.driver_code"
                 class="f1-metric-card flex items-center"
                 :class="[getTeamCardClass(driver.team), getPositionCardClass(index + 1)]">
...
const getPositionCardClass = (position: number): string => {
  ...
  if (position === 2) return 'ring-2 ring-mercedes-silver/50 shadow-lg shadow-mercedes-silver/20'
  if (position === 3) return 'ring-2 ring-racing-orange/50 shadow-lg shadow-racing-orange/20'
  return ''
}
...
// Debug function to check if team mapping works
const debugTeamMapping = (teamName: string): void => {
  console.log('Team:', teamName)
  console.log('Slug:', getTeamSlug(teamName))
  console.log('Card Class:', getTeamCardClass(teamName))
  console.log('Indicator Class:', getTeamIndicatorClass(teamName))
  console.log('Value Class:', getTeamValueClass(teamName))
}
</script>
